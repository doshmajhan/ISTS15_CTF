...
int main() {
    // The bit state of the cipher stream
    //c_size state = 0xbadcaffec0d31337;
    // The cipher polynomial
    //c_size p = 0x010004200300208A;
    // The feedback stream
    //const c_size p = 0b00010110;
    c_size state = 0b10101001;
    c_size feedback = 0; 

    char* ptext = "flag{lolz}";
    uint8_t ctext[strlen(ptext) + 1];
    for (unsigned long i=0; i<strlen(ptext); i++) {
        ctext[i] = (uint8_t)(getbyte(state, feedback) ^ ptext[i]);
    }
